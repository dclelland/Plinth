...
Plinth's philosophy is to do "the simplest thing that works". For example, many of the arithmetic/numeric functions are implemented twice – for `Matrix<Float>` and `Matrix<Double>` – instead of managing this through a morass of different typeclasses and implementations, we just copy and paste the implementation and change the type argument.
